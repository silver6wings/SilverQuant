...
* 项目经历实盘考验，注重工业级的稳定持续运行，最长曾达半年无人干预稳定运行交易模式
* 项目组件化设计思路注重策略实现的灵活性，在稳定的基础上努力减少框架定制带来的束缚
* 项目针对同一个三方依赖会提供多个解决方案，尽可能避免由上游因素导致的研发沉没成本
...
使用模拟盘（`IS_PROD = False`）时，除配置 `GM_XXX` 外，还需在运行策略前打开掘金3客户端，并登录后在终端中连接对应的仿真账户（`credentials.py` 里配置的 `GM_ACCOUNT_ID`），确认交易服务已连接后再启动脚本

### 配置 Credentials

> 复制项目根目录下的`credentials_sample.py`，改名为`credentials.py`并填入自己的参数
> 
...
> 5. 配置到`credentials.py`
> 6. 每次运行模拟盘策略前，需打开掘金客户端并连接该仿真账户，否则无法下单
...
> 3. 模拟盘（`IS_PROD = False`）：确认掘金客户端已打开并已连接仿真账户；实盘（`IS_PROD = True`）：确认 QMT 交易软件正在运行
...
pywencai 近期不太稳定，可尝试使用 [pywencai-enhanced](https://github.com/HeRiki/pywencai-enhanced) 作为替代：
```
...
其次，检查自己的选股提示词 (Prompt) 能不能在网页上搜到票
...
框架支持多个 token，请参考 credentials_sample.py TUSHARE_TOKEN
注意tushare日线只有免费的不复权数据，复权数据需要付费，注意复权对于策略的影响
```
